...
type SimulationResult = {
  ...
  revenue_base_bill_savings: number;
...
const DEFAULT_TARIFFS: TariffPreset[] = [
  ...
  { id: 7, name: '농사용(을) 고압A', baseRate: 1210, savings: 67.93 },
];
...
export const useProposalStore = create<ProposalState>((set, get) => ({
  ...
  getSimulationResults: () => {
      ...
      monthlyData.forEach((data) => {
        ...
        if (data.peakKw > 0) {
          baseBillSaving = Math.max(
            0,
            data.baseBill - state.baseRate * data.peakKw
          );
        } else {
    ...
    // [정책] 자연감소 미적용 — 임대/구독/수익배분형은 단순 곱하기 20 (사용자 결정 2026-05-20)
    ...
    // [정책] 자연감소 미적용 — 단순 곱하기 (사용자 결정 2026-05-20)
